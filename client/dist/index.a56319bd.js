...
const createListElem = (listElements, list)=>{
    for (const listEl of listElements){
        ...
        if (window.location.href === BASE_LOCATION) listElem.insertAdjacentHTML("afterbegin", createProjectElem(listEl));
        else {
...
const renderTasksByStatus = async ()=>{
    ...
    if (todoTodos) createListElem(todoTasks, taskListTodo);
    if (inReviewTasks) createListElem(inReviewTodos, taskListInReview);
